...
Usage:
  ...
  python3 merge_graph.py --rank .mermaid/current/
  ...
  python3 merge_graph.py --render .mermaid/current/
...
def _parse_option(args, name, default=None):
    if name not in args:
        return default
...
def _parse_repeated_option(args, name):
    values = []
    index = 0
    while index < len(args):
        if args[index] == name:
...
def _source_from_tail(args, default=".mermaid/current/"):
    skip_next = False
    positional = []
    options_with_values = {"--text", "--summary", "--to", "--session", "--author", "--type", "--evidence"}
    for token in args:
        if skip_next:
            skip_next = False
            continue
    ...
    return positional[-1] if positional else default
...
def main():
    ...
    if command == "--details":
        if len(args) < 2:
            raise ValueError("--details requires a node id")
        node_id = args[1]
        source = args[2] if len(args) > 2 else ".mermaid/current/"
        print(node_report(load_source(source), node_id))
        return
    ...
    if command == "--rank":
        source = args[1] if len(args) > 1 else ".mermaid/current/"
        print(rank_report(load_source(source)))
        return
    ...
    if command == "--render":
        ...
        if not target.is_dir():
            raise ValueError("--render requires a directory source")
        target.mkdir(parents=True, exist_ok=True)
        layer_files = {
            "R": "requirements.mmd",
            "D": "data.mmd",
            "F": "files-functions.mmd",
            "U": "ui.mmd",
        }
        written = {}
        for layer, filename in layer_files.items():
            path = target / filename
            path.write_text(graph.to_mermaid(layer=layer), encoding="utf-8")
            written[layer] = str(path)
        all_path = target / "all.mmd"
        all_path.write_text(graph.to_mermaid(), encoding="utf-8")
        written["all"] = str(all_path)
        print(json_dump({"rendered": written}))
        return
    ...
    if command == "--validate":
        ...
        if issues:
            hard_issues = [item for item in issues if not str(item).startswith("warning: ")]
            warnings = [item for item in issues if str(item).startswith("warning: ")]
            if hard_issues:
                print("ISSUES FOUND:")
    ...
    if command == "--comment":
        if len(args) < 2:
            raise ValueError("--comment requires a node id")
        node_id = args[1]
        text = _parse_option(args[2:], "--text")
        if not text:
            raise ValueError("--comment requires --text")
        source = _source_from_tail(args[2:])
        graph = load_source(source)
        ...
        comment = {
            ...
            "session": _parse_option(args[2:], "--session", os.environ.get("AIDS_AGENT_ID") or os.environ.get("USER")),
            "author": _parse_option(args[2:], "--author", "codex"),
            "type": _parse_option(args[2:], "--type", "execution"),
            "body": text,
            "evidence": _parse_repeated_option(args[2:], "--evidence"),
        }
        node.comments.append(comment)
        write_normalized_graph(source, graph)
        print(json_dump({"comment": comment}))
        return

    if command == "--iterate":
        if len(args) < 2:
            raise ValueError("--iterate requires a node id")
        node_id = args[1]
        source = _source_from_tail(args[2:])
        graph = load_source(source)
        node = graph.get_node(node_id)
        ...
        to_status = _parse_option(args[2:], "--to")
        if to_status:
            node.status = to_status
        iteration = {
            "at": datetime.now(timezone.utc).astimezone().isoformat(timespec="seconds"),
            "session": _parse_option(args[2:], "--session", node.session or os.environ.get("AIDS_AGENT_ID") or os.environ.get("USER")),
            "from_status": previous,
            "to_status": node.status,
            "summary": _parse_option(args[2:], "--summary", ""),
            "evidence": _parse_repeated_option(args[2:], "--evidence"),
        }
        node.iterations.append(iteration)
        write_normalized_graph(source, graph)
        print(json_dump({"iteration": iteration}))
        return

    if command == "--claim":
        if len(args) < 2:
            raise ValueError("--claim requires a node id")
        node_id = args[1]
        source = _source_from_tail(args[2:])
        graph = load_source(source)
        node = graph.get_node(node_id)
        if not node:
            raise ValueError(f"node not found: {node_id}")
        if node.session:
            raise ValueError(f"node already claimed by {node.session}: {node_id}")
        blockers = graph.blockers(node_id)
        if blockers:
            raise ValueError(f"node has blockers: {', '.join(item.id for item in blockers)}")
        import os
        session = _parse_option(args[2:], "--session", os.environ.get("AIDS_AGENT_ID") or os.environ.get("USER") or "unknown-session")
        node.session = session
        node.status = "doing"
        node.iterations.append({"summary": _parse_option(args[2:], "--summary", "Claimed for execution."), "from_status": "todo", "to_status": "doing", "session": session})
        write_normalized_graph(source, graph)
        print(json_dump({"claim": {"node": node_id, "session": session}}))
        return

    if command == "--done":
        if len(args) < 2:
            raise ValueError("--done requires a node id")
        node_id = args[1]
        source = _source_from_tail(args[2:])
        graph = load_source(source)
        node = graph.get_node(node_id)
        ...
        previous = node.status
        node.status = "done"
        node.iterations.append({"summary": _parse_option(args[2:], "--summary", "Completed."), "from_status": previous, "to_status": "done", "session": _parse_option(args[2:], "--session", node.session)})
        write_normalized_graph(source, graph)
        print(json_dump({"done": {"node": node_id, "next": [item.to_dict() for item in graph.next_after(node_id)]}}))
        return
